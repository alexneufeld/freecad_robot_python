...
class Robot6Axis:
        ...
        for i in range(6):
            temp.addSegment(
                PyKDL.Segment(
                    ...
                    PyKDL.Frame.DH(
                        KinDef[i].a,
                        KinDef[i].alpha * (math.pi / 180),
                        KinDef[i].d,
                        KinDef[i].theta * (math.pi / 180),
                    ),
                )
            )
    ...
    def setTo(self, To: FreeCAD.Placement) -> bool:
        fksolver1 = PyKDL.ChainFkSolverPos_recursive(
            self._Kinematic
        )  # Forward position solver
        iksolver1v = PyKDL.ChainIkSolverVel_pinv(
            self._Kinematic
        )  # Inverse velocity solver
        iksolver1 = PyKDL.ChainIkSolverPos_NR_JL(
            self._Kinematic, self._Min, self._Max, fksolver1, iksolver1v, 100, 1e-6
